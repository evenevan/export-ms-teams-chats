...
function Get-EncodedProfilePicture($userId, $imageFolderPath, $clientId, $tenantId) {
    if ($null -eq $userId) {
    ...
    if (-not(Test-Path $profilePictureFile)) {
        Write-Verbose "Profile picture cache miss, downloading."
        ...
        try {
            ...
            $encodedProfilePicture = ConvertTo-Base64Image $profilePictureFile
        }
        catch {
            Write-Verbose "Unable to fetch and encode profile picture, using default profile picture."
            $encodedProfilePicture = $defaultProfilePicture
        }
    }
    else {
